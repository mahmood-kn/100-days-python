...
def check_pc_status(pc,user_score):
  pc_score=sum(pc)
 
  if pc_score<17:
    new_pc_card=random.choice(cards)
    pc.append(new_pc_card)
    print(f"Computer gets new card, Computer new cards:{pc}, new card is {new_pc_card}, current score is {sum(pc)}")
    check_pc_status(pc,user_score)
  else:
    if pc_score==21:
      print(f"You lose, Computer Blackjack")
    elif pc_score>21:
...
def check_user_status(user,pc):
  ...
  if user_score>21:
    ...
    if hit=='y':
      ...
      print(f"Your cards:{user}, new card is {new_card}, current score is {sum(user)}")
...
while continue_play:
  ...
  if want_play=='y':
    ...
    user_score=sum(start_cards['user_cards'])
    pc_score=sum(start_cards['pc_cards'])
